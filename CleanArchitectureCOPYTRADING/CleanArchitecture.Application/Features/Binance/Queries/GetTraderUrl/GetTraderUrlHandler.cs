...
namespace CleanArchitecture.Application.Features.Binance.Queries.GetTraderUrl
{
    public class GetTraderUrlHandler : IRequestHandler<GetTraderUrlQuery, List<string>>
    {
        ...
        public GetTraderUrlHandler(IBinanceTraderTypeDataService binanceTraderTypeDataService,
            IBinanceTraderService binanceTraderService, IBinanceTraderPerformanceService loadTraderPerformanceService)
        {
        ...
        public async Task<List<string>> Handle(GetTraderUrlQuery request, CancellationToken cancellationToken)
        {
            ...
            var auditRault = (await _loadTraderPerformanceService.GetAllTraderDataPerformanceBinanceForModelAudit(dataDicBinanceTrader));

            var allTraderDataPerformanceBinanceForModelAuditEtStock = auditRault
                .OrderByDescending(p => p.IAIsTopTraderScore)
                .GroupBy(p => p.EncryptedUid).ToDictionary(p => p.Key, p => p.Where(p => p.IAIsTopTraderScore >= 0.2).Count()).Where(p => p.Value > 1).Select(p => new { p.Key, p.Value }).ToList();

            allTraderDataPerformanceBinanceForModelAuditEtStock= allTraderDataPerformanceBinanceForModelAuditEtStock.Where(p => !NotIncludeGuidTrader.NotIncludeGuid.ContainsKey(p.Key)).ToList();

            
                 List<string> fileJson = Directory.GetFiles(_pathDataJson, "*.*").Select(p=> new FileInfo(p)).Select(p=>p.Name).Select(p => p.Split('_').FirstOrDefault()).ToList();

            hebdomadaire.AddRange(allTraderDataPerformanceBinanceForModelAuditEtStock.Select(p=>p.Key).ToList());
            return hebdomadaire.Distinct().Where(p=>!fileJson.Contains(p)).ToList();
